...
// -----------------------------------------------------------------------------
// GEODE TABLE  – memoised coefficients (no change to logic needed here)
// -----------------------------------------------------------------------------
class GeodeTable {
public:
        ...
        : deg_param_size_(degree_of_P), Fmax_(Fmax_val) // deg_param_size_ is size of m-vector for t_k
    {
        ...
        for (unsigned F_val = 1; F_val <= Fmax_; ++F_val) { // Renamed F to F_val to avoid conflict if needed
            build_shell(F_val);
        }
    }
...
public:
    ...
    explicit RootSolver(const PolyF& P_in) : P_(P_in), d_poly_degree_(P_in.degree()) {
        ...
        if (d_poly_degree_ >= 1 && c_coeffs_WR[1] == Float(0L)) {
            ...
            // For now, we might throw or indicate this solver cannot proceed.
            std::cerr << "Warning: Coefficient c1 (for W&R formula) is zero. Solver may fail or give incorrect results." << std::endl;
            // You might decide to let it proceed if c0 is also 0, then x=0 is a root, factor it out.
        }
        build_WR_t_values();
    }
